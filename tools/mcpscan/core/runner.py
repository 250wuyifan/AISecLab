...
from .extract_description import *
import json
from textwrap import dedent
import re, ast

# MCPScan 资源目录（规则、提示词模板）
_MCPSCAN_ROOT = Path(__file__).resolve().parent.parent  # tools/mcpscan/


console = Console()
llm: LLMClient = None  # 延迟初始化，由 run_scan() 设置
...
def init_llm(
    ...
    console.print(f":brain: LLM: {llm}", style="bold cyan")
...
def run_scan(
    ...
    # 初始化 LLM 客户端（如尚未初始化或需要重新配置）
    global llm
    if llm is None:
        init_llm(provider=llm_provider, model=llm_model, api_key=llm_api_key, base_url=llm_base_url)
    ...
    semgrep_cfg = _MCPSCAN_ROOT / f"rules/semgrep-security-{lang}.yml"
    ...
    if monitor_code:
        ...
        if high_risk_files:
            ...
            PROMPTS_DIR = _MCPSCAN_ROOT / "prompts"
